...
module.exports = {
  ...
  create: function(context) {
    ...
    return {
      CallExpression: function(node) {
        if (isLodashMap(node) && hasTwoArguments(node)) {
          ...
          if (
            isDefinedAs(collectionNode, 'ObjectExpression') ||
            isPartOfConditionalReplacement(collectionNode)
          ) {
    ...
    function isPartOfConditionalReplacement(node) {
      // TODO: detect that collection variable is argument of Array.isArray() more accurately
      if (node.parent && node.parent.type === 'ConditionalExpression') {
        return true;
      }
      if (node.parent) {
        return isPartOfConditionalReplacement(node.parent);
      }
      return false;
    }
  },
